...
`install.sh` は以下を行う。
1. プラグインマネージャの準備（lazy.nvim は nvim 初回起動時に自動導入されるため事前準備は不要）
...
設定は Lua で記述している（`nvim/init.lua` + `nvim/lua/` 以下）。プラグイン管理は lazy.nvim。
現在のプラグイン構成は Neovim 0.11 以上を前提にしている。Ubuntu 24.04 の apt 版
Neovim 0.9.5 では `blink.cmp` / `nvim-lspconfig` / `telescope.nvim` などが起動時に
バージョンエラーになるため、公式 tarball / AppImage / PPA などで新しい Neovim を入れる。
...
- Docker (`sand` コマンドでサンドボックスを使う場合)
- vim / neovim 0.11+ エディタ
